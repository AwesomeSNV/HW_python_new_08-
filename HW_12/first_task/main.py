...
class Student:
    ...
    def add_exam_result(self, course_name, value):
        if course_name not in self._course_exams.keys():
            raise ValueError(f'Студент {self.get_full_name} не записан на такой курса')
...
stu1.add_test_result(course, 45)
stu1.add_test_result(course, 73)
...
course = "Литература"
stu1.add_test_result(course , 98)
stu1.add_test_result(course , 76)
print_res(stu1, "test", course)
